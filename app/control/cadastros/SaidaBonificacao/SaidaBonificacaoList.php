...
class SaidaBonificacaoList extends TPage
{
  private $form;
  private $datagrid;
  ...
  private static $formName = 'form_SaidaBonificacaoList';
  ...
  public function __construct()
  {
    ...
    $this->setDefaultOrder('id_saida', 'asc');
    $this->setLimit(10);

    $this->addFilterField('id_fichacadastral', '=', 'id_fichacadastral');
    $this->addFilterField('dt_saida', '=', 'dt_saida');

    //Criação do formulario 
    $this->form = new BootstrapFormBuilder(self::$formName);
    $this->form->setFormTitle('Saidas de Bonificações');
    // expand button
    $this->form->addExpandButton();

    //Criação de fields
   
    $campo1 = new TDBUniqueSearch('id_fichacadastral', 'sample', 'FichaCadastral', 'id_fichacadastral', 'nm_fichacadastral');
    $campo2 = new TDate('dt_saida');



    $this->form->addFields([new TLabel('Pessoa')], [$campo1]);
    $this->form->addFields([new TLabel('Data da saida')], [$campo2]);
    ...
    $this->form->addActionLink(_t('New'), new TAction(['SaidaBonificacaoForm', 'onEdit'], ['register_state' => 'false']), 'fa:plus green');
    ...
    $column_1 = new TDataGridColumn('id_saida', 'Codigo', 'center');
    $column_2 = new TDataGridColumn('ficha_cadastral->nome', 'Pessoa', 'center',);
    $column_3 = new TDataGridColumn('dt_saida', 'Data da Saida', 'center',);
    $column_4 = new TDataGridColumn('vl_ecototal', '$Eco', 'center',);
    $column_5 = new TDataGridColumn('vl_saldo', 'Saldo', 'center',);
    ...
    $column_5->setTransformer(function ($value, $object, $row, $cell = null, $last_row = null) {
      if (is_numeric($value)) {
        return 'R$ ' . number_format($value, 2, ',', '.');
      }
      return $value;
    ...
    $column_1->setAction(new TAction([$this, 'onReload']), ['order' => 'id_saida']);
    $column_2->setAction(new TAction([$this, 'onReload']), ['order' => 'id_fichacadastral']);


    $action1 = new TDataGridAction(['SaidaBonificacaoForm', 'onEdit'], ['id' => '{id_saida}', 'register_state' => 'false']);
    $action2 = new TDataGridAction([$this, 'onDelete'], ['id' => '{id_saida}']);
  ...
  public function onDelete($param = null)
  {
    ...
    if (isset($param['delete']) && $param['delete'] == 1) {
      try {
        ...
        $object = new SaidaBonificacao($key, FALSE);

        SaidaBonificacaoService::excluir($object, $conn);
